...
describe("summarisePhaseEffects", () => {
  describe("anchor shifts", () => {
    it("surfaces a per-good anchor up (>1) as 'X demand up'", () => {
      expect(summarisePhaseEffects(phase([anchor(1.5, "fuel")]))).toBe(
        "Fuel demand up",
      );
    });

    it("surfaces a per-good anchor down (<1) as 'X demand down'", () => {
      expect(summarisePhaseEffects(phase([anchor(0.56, "ore")]))).toBe(
        "Ore demand down",
      );
    });

    it("surfaces a null-goodId anchor up as 'All demand up'", () => {
      expect(summarisePhaseEffects(phase([anchor(3.0, null)]))).toBe(
        "All demand up",
      );
    });

    it("surfaces a null-goodId anchor down as 'All demand down'", () => {
      expect(summarisePhaseEffects(phase([anchor(0.5, null)]))).toBe(
        "All demand down",
      );
    });
    ...
    it("merges multiple same-direction goods into one phrase", () => {
      expect(
        summarisePhaseEffects(phase([anchor(1.6, "food"), anchor(1.4, "medicine")])),
      ).toBe("Food, Medicine demand up");
    });

    it("treats an identity anchor (value === 1) as no demand change", () => {
      expect(summarisePhaseEffects(phase([anchor(1, "fuel")]))).toBe(
    ...
    it("keeps opposite-direction per-good phrases alongside an all-goods phrase", () => {
      ...
      expect(
        summarisePhaseEffects(phase([anchor(0.5, null), anchor(1.5, "fuel")])),
  ...
  describe("rate multipliers", () => {
    it("surfaces production_rate < 1 as 'Production slowed'", () => {
      ...
      expect(summarisePhaseEffects(phase([mod]))).toBe("Production slowed");
    });
    ...
    it("surfaces production_rate > 1 as 'Production boosted'", () => {
      ...
      expect(summarisePhaseEffects(phase([mod]))).toBe("Production boosted");
    });
  });
  ...
  describe("composition and edge cases", () => {
    it("joins multiple effect categories with ' · ' in a stable order", () => {
      ...
      expect(
        summarisePhaseEffects(phase([anchor(2.5, "fuel"), production])),
