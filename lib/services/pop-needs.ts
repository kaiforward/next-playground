 ...
 */
export function systemPopNeeds(systemId: string, basis: CivilianDemandBasis, governmentType: GovernmentType): PopNeedData[] {
  return computePopNeeds(basis, governmentType, marketsBySystem().get(systemId) ?? []).map((n) => ({
    ...n,
