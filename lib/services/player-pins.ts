 ...
 * the system currently exists — a pin surviving past abandonment is filtered on read
 * (`lib/services/tracker.ts`), not rejected on write. `pinnedSystemIds` is insertion-ordered and
 * deduplicated here so no reader has to.
 */
...
export function setSystemPin(input: PinInput): SetSystemPinResult {
