...
import { isGovernmentType } from "@/lib/types/guards";
...
describe("HOME_SYSTEM_PREFAB", () => {
  ...
  it("is self-sufficient at every government — production meets the full tick-model consumption for every good it manufactures", () => {
    // Consumption must match the live per-tick model (consumptionRate): the per-capita baseline, the
    // technician/engineer skilled baskets, AND the faction government's consumption boost. Each government's
    // capital is sized to its own boosted demand, so a technocratic (electronics) or theocratic (textiles)
    // homeworld covers its boosted goods day-one — not just frontier (which has no boost). A fully-staffed
    // capital works exactly its licensed skilled demand, so its basis is the labour allocation over its base.
    for (const governmentType of Object.keys(GOVERNMENT_TYPES).filter(isGovernmentType)) {
      const buildings = computeHomeworldBuildings(HOME_SYSTEM_POP, governmentType);
      ...
      for (const g of Object.keys(OUTPUT_PER_UNIT)) {
        ...
        const consumption = consumptionRate(g, basis, governmentType) + (recipeDraw[g] ?? 0);
        expect(production, `${governmentType}/${g}`).toBeGreaterThanOrEqual(consumption - 1e-6);
      }
    }
  });

  it("is deterministic — recomputing yields an identical stamp for a given government", () => {
    expect(computeHomeworldBuildings(HOME_SYSTEM_POP, "frontier")).toEqual(b);
  });
});

describe("homeworldGardenBody", () => {
  it("guarantees enough deposit slots for every extractor the prefab places", () => {
    const garden = homeworldGardenBody("frontier");
  ...
  it("guarantees enough habitable space for the prefab's housing", () => {
    ...
    expect(homeworldGardenBody("frontier").habitableSpace).toBeGreaterThanOrEqual(housing * effectiveSpaceCost(HOUSING_TYPE));
  });
  ...
  it("guarantees enough general space for every factory, academy and housing level", () => {
    ...
    expect(homeworldGardenBody("frontier").generalSpace).toBeGreaterThanOrEqual(generalFootprint);
  });
});
