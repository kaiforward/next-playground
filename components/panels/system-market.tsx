...
/**
 * Moved from `app/(game)/@panel/system/[systemId]/market/page.tsx`. `useMarket` returns a
 * zero-default with no discriminant of its own (ledger note from the Task 7/8 review) — the empty-
 * market branch below is a DEVELOPED-TIER check ("this system has no market yet"), not a not-found
 * check; a stale/absent systemId never reaches this component at all, since `system-panel.tsx`'s
 * root already turned that case into the not-found state before any tab body mounts.
 */
export function SystemMarket({ systemId }: { systemId: string }) {
