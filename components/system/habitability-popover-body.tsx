...
export function HabitabilityPopoverBody({
  ...
  if (fillOrder.length === 0) {
    return <p className="text-text-secondary">No habitable-land bodies yet.</p>;
  }
  return (
    <div className="space-y-1">
      <p className="whitespace-nowrap font-mono text-text-primary">Habitability: {habitabilityPct}%</p>
      ...
      <ul className="space-y-0.5 overflow-x-auto">
        {fillOrder.map((row, i) => (
          <li key={i} className="flex items-center justify-between gap-3 whitespace-nowrap">
