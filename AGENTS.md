...
- **Never a merge commit, and never PR implementation detail in the target's history — which is why a feature branch squashes.** A feature or bugfix branch lands as a single commit describing what the thing does; the commits that built it — spec revisions, roadmap bookkeeping, review-finding closures, phase check-ins, bugfixes to the branch's own work — are the making of it and are irrelevant once it lands. That churn is what "noise" means here, not broken builds: this project's pipeline produces it on every branch by design, so the answer for a feature branch is always squash. Fast-forwarding is not itself the problem and `main` deliberately allows rebase — a long-running `shared/*` branch whose commits are genuinely atomic and each worth reading may land that way. Merge through a PR, never a local merge onto the target — the commit hook blocks direct commits on `shared/*` and `main`, and a local squash-merge is a direct commit.
