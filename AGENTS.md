...
- **The mirror of that rule has already cost more than the rule itself, and a "ruled out" is where it lands.** Phantom `MIN_DEMAND` demand was written off as "a one-time founding tax" on the strength of its 416-cycle read (0.3% of deliveries) and recorded as **RULED OUT** in `docs/BACKLOG.md` — where it sat, believed, steering the `luxuries` investigation away from its own cause. Re-measured at 42 cycles it was **24.7% of all delivered cargo** and over 90% of the haul of the scarce advanced goods; fixing it (#211) took `luxuries` consumer cover 0.02 → 0.81 and consumers-empty 53% → 37%. A startup fault is invisible at equilibrium *by definition*, so an equilibrium read can never be evidence that a founding pathology does not exist. **Hold a negative result to the same both-horizon evidence bar as a positive one, and record which horizon and cohort it was measured at** — a "ruled out" is the most durable thing you can write, because nobody re-tests it.
