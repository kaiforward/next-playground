...
| Slug | Flags (read AGENTS.md for the rule) |
|------|-------------------------------------|
| `world-not-serialisable` | a `Map`/`Set`/`Date`/class instance or `Infinity`/`NaN` stored in `World` state (breaks JSON save/load) |
...
- **`world-not-serialisable`** — flag a `Map`/`Set`/`Date`/class instance or a possible `Infinity`/`NaN` assigned into a `World` row / `meta`. Ordinary local `Map`/`Set` inside a processor body (not persisted into `World`) is fine.
