...
- **`duplicate-implementation`** — the candidate pairs arrive from `npm run duplication` under `## Duplication candidates`; the script searches, you adjudicate. The bar is **same decision, same medium, same lifecycle, same posture** — not "both touch storage". Two thirds of a repo-wide sweep's candidates fail it. The worked rejection: a `sessionStorage` helper and a `localStorage` hook share a try/catch + `JSON.parse` + narrow, but differ in medium (die-with-the-tab vs still-true-tomorrow), in kind (no `useState`, no hydrate-in-effect, no SSR guard) and in fallback posture — the shared shape is what the boundary forces on any reader, not a shared decision. Also never flag: per-processor read views (`docs/active/engineering/processor-architecture.md` sanctions them), one-file-per-route bodies where `withServiceErrors` is already the extraction, or one-expression idioms (`Math.round(x * 100)`) embedded in different domain rules. Repeated arrange blocks in tests are usually right; flag a test pair only when the copied thing is logic. Report a pair the script missed if you see one — its window is 6 normalised lines, so a shorter copy or a reordered one is invisible to it.
