...
- **Pure logic outside `lib/`** — framework-free helper modules elsewhere are unit-tested too. The clearest example is the Pixi map's pure math/geometry/LOD modules (e.g. `components/map/pixi/lod.ts` has `__tests__/lod.test.ts`). These have no React/Pixi-instance dependency and should be tested like engine code. Note: the node project has **no DOM**, and jsdom has no WebGL — modules that instantiate Pixi display objects CAN'T be unit-tested directly, so don't demand tests for a `SystemObject`-style class; focus on the pure functions and exported constants/thresholds.
- **Component tests** — a `.test.tsx` file renders in jsdom with Testing Library; a `.test.ts` file is pure logic in node. A component test asserts roles, accessible names and text, never classes or styles (a style assertion restates a prop and breaks on a rename while the thing may be invisible). Where a number's only observable is a style, the maths belongs in a helper tested in node.
