...
- Do not treat clearly-deferred or explicitly out-of-scope items as gaps — *when the deferral is justified where it is made*. A spec or diff comment saying "later" is respected on its own terms.
  **But a deferral is itself reviewable when the diff defers something it was already touching.** A new `docs/BACKLOG.md` row describing a cheap, self-contained problem in a file this same diff edits is a finding, category `deferred-in-reach` — the project's rule is that such findings are fixed in-task (AGENTS.md, "Review process"). Report it `info`, or `major` if the commit message claims the findings were fixed. Judge cheapness by the judgement required, not the line count: a compiler-guided mechanical sweep is cheap however many sites it touches.
