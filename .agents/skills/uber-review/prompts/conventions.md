...
Many of these rules are mechanical — a forbidden call or pattern that's simply present or not (`as` cast, `.includes()` on a Record, a static `fs`/`process.env` import in the pure path). Flag those. A few gotchas need data-flow reasoning (does a value reach `World` state, serialisation/determinism safety, swallowed errors); leave those to the world-integrity / silent-failures reviewers, who receive the same rules.
