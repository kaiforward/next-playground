...
- **[S] Rescale goods-flow units for whole-unit trading headroom** — Per-tick goods flows land sub-1 even on high-pop worlds (a "500M" world is `population ≈ 500` since 1 abstract unit = 1M people; small worlds drop under the 0.1 display floor), so one whole traded unit ≈ 1M-people-worth of a good — many ticks of a small world's entire output. Multiply every goods-side quantity by one constant K (`OUTPUT_PER_UNIT`, `GOOD_CONSUMPTION`, the `*_STORAGE_PER_UNIT` constants, and existing market `stock`); equilibrium is invariant because price/days-of-supply/idle/decay are all ratios. Population axis untouched. `stock`/`population` are `Float` (no Int-overflow risk) but verify nothing in the consumption path floors tiny-world flows to 0 (`MarketGood.dailyNeed` is noted floored). Best landed *with or just before* player trading — trading is what consumes the unit scale, so pick K against that, not in a vacuum.
