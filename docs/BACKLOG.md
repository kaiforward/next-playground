...
- **[M] Global economy-scale knob (= sub-project 1 of the trade-rework initiative)** — One modifiable `ECONOMY_SCALE` that uniformly multiplies every goods-side quantity (production `OUTPUT_PER_UNIT`/`OUTPUT_OVERRIDES`, `GOOD_CONSUMPTION`, the storage-capacity term, and market `stock`), so per-tick flows stop landing sub-1 and a whole traded unit stops being "1M-people-worth." Equilibrium is **invariant** because price/days-of-supply are ratios (`stock`/`targetStock` both scale) — *above the `MIN_DEMAND` floor*; the work is the **audit of the absolute (non-ratio) terms** that must scale explicitly — `MIN_DEMAND` (demand floor), the per-unit storage constants, and **both** silent-flow budgets (`FLOW_BUDGET` for market diffusion + `GENERATION_PER_POP` for directed logistics) + seed-stock rounding — but *not* the matcher's route/fuel cost (goods-agnostic; scaling it is the wrong lever) or the mission reward formula (deferred to the contract rework). Full inventory in the design doc. `stock`/`population` are `Float` (no Int-overflow). Calibrate the factor via the simulator. Foundational + independently mergeable. **Full design + 3-dial model + decomposition: [economy-scaling-and-trade-rework.md](./planned/economy-scaling-and-trade-rework.md); roadmap slot in [economy-simulation-vision.md](./planned/economy-simulation-vision.md) §13.**
...
- **[S] Flow-overlay particle thresholds vs economy-scale** — The map flow-overlay particle density (`LOGISTICS_FLOW` / `TRADE_FLOW` in `components/map/pixi/theme.ts`: `volumePerExtraParticle`, `minParticlesPerEdge`, `maxParticlesPerEdge`, `maxTotalParticles`) is tuned for S=1 flow magnitudes and is intentionally **not** scaled by `ECONOMY_SCALE` (client-side visual constants; the knob is server-only by design). At a large calibrated S (sub-project 2) every edge pins at `maxParticlesPerEdge` and the global budget concentrates on the top flows, so the overlay loses its high- vs low-volume contrast (the particle count still saturates safely — purely a legibility loss, not a perf/correctness issue). Revisit the thresholds once calibration picks the real S so density stays meaningful. Verified fine at S=10 during the SP1 smoke. Blocked on / part of sub-project 2 calibration.
