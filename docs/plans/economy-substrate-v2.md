...
- [ ] **7.5 — Display-bug fixes (playtest notes #5a, #7).**
...
- [ ] **7.6** Any other UI polish surfaced during the milestone.
- [ ] **7.7** Commit per component. Phase PR → shared branch.

**Phase 7 gate:** panel renders the real substrate + per-market band + honest cadence; #5a/#7 fixed; visibility-gating preserved (unsurveyed → `{ visibility: "unknown" }`); static vs tick-dynamic split preserved (substrate `staleTime: Infinity`, industry tick-invalidated; the countdown is tick-derived, not a new query).
