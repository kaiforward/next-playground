...
Baseline files go in this session's scratchpad — they are large, disposable, and **never committed**. Read the path from your system prompt; it looks like `C:/Users/<user>/AppData/Local/Temp/claude/C--source-next-playground/<session-id>/scratchpad`.

**Do not use a shell variable for it.** Shell state (`export`, functions) does **not** persist between Bash tool calls in this harness — only the working directory does. An `export SCRATCH=...` in one call resolves to empty in the next, silently writing to `/baseline.txt`. **Substitute the literal path everywhere `<SCRATCH>` appears below**, in every command.

```bash
mkdir -p "<SCRATCH>" && ls -d "<SCRATCH>"
```

Expected: the path echoes back.
