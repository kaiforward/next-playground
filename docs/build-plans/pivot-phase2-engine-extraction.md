...
- Modify: `lib/engine/simulator/economy.ts`, `lib/engine/simulator/types.ts`, `lib/engine/simulator/world.ts`, `lib/engine/simulator/runner.ts` (+ analysis modules where field paths move)
...
- [ ] **Step 4: Converge the simulator.** `SimWorld` is replaced by `World` + a sim wrapper `SimState = { world: World; players: SimPlayer[]; …sim-only fields }`; `createSimWorld` becomes a thin wrapper over `generateWorld` plus bot/player seeding; `simulateWorldTick` is deleted in favor of `runWorldTick` (bot ticks stay in the runner, after the world tick, as today). Update sim analysis modules for moved field paths. **Coherence, not parity** (project rule): sim output needn't be bit-identical to pre-refactor; `npm run simulate` must stay healthy (no NaN, greedy ≫ random, dispersion sane).
