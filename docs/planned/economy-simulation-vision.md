...
PRICE      → emerges from cycles-of-supply (see §6)
```
...
| **Pricing anchor** | Global per-good magic constant (`targetStock`) | Emergent per-system **cycles-of-supply**. Anchor concept deleted. |
...
2. **Demand & consequence loop** — population-scaled consumption, need-satisfaction recording, unrest, growth/decline, migration along lanes; cycles-of-supply pricing. This delivers the "living world." **Specced + forks resolved (SP2, shipped):** Notable resolutions: it gains a **Part 0** that *de-regions the diffusion engine* (separating performance-sharding from the gameplay flow boundary — flow now follows **sovereign/faction** borders + distance, not regions); **prosperity is retired** in favour of `unrest`; and **rebellion + relation-weighted borders are deferred to sub-project 5** (they need faction control to be a live actor). Tiny/single-system factions noted as a future-consideration item.
...
- **Pricing formula**: exact cycles-of-supply curve, elasticity, and how the round-trip guard (slippage + spread) carries over.
