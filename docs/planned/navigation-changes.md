...
Changes to the active navigation system ([navigation.md](../active/gameplay/navigation.md)) driven by the ship roster expansion and faction system. This is a delta document — it describes what changes, not the full system. When implemented, merge these changes into the active navigation doc.
...
- **`shipSpeed`** — the ship's speed stat (lower is faster, per [ship-roster.md §1.1](../active/gameplay/ship-roster.md)). Modified by Thruster Upgrade module
...
Stages 1 and 4 cause hull damage alongside cargo loss when incidents trigger. This connects the danger pipeline to the ship damage system ([ship-roster.md §5.3](../active/gameplay/ship-roster.md)):
