...
2. **SP5 autonomic-light agency, brought forward — re-sequenced seed-coherence-first (2026-06-26).** Building logistics surfaced that the seeder *hand-authors a mature base no agency built* — the recurring "seed bug" root — so SP5 starts by fixing *generation*. **Stage 1 — seed-coherence foundation:** seed a few self-sufficient subsistence cores per faction on an inert frontier (the seeder makes no agency decision); the *build planner* owns industry-allocation; an *age-forward snapshot harness* grows the mature galaxy from the live agency; autonomic-light *build-up* is the recovery counterforce (self-funded, slow, no treasury); *validate logistics on the grown world*. **Stage 2 — emergent territory:** un-owned space + dynamic membership + colonisation. The at-risk middle still needs food *moved* not rebuilt — logistics + build compose. This makes build load-bearing for generation itself: the ultimate **"make the baseline honest, not efficient"** move — stop hand-authoring the mature state, grow it from rules.
