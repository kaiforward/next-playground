...
| Mature systems over-produce their specialty; the unexported surplus reads as scale-invariant "selling" idle decay can't shed | Directed faction logistics — the *supply side* of the same lever: move the surplus out, local uptake recovers |
...
The audit resequences the [vision §13](./economy-simulation-vision.md#13-sub-project-decomposition) order. See that section's 2026-06-28 **"CURRENT SEQUENCE"** callout for the canonical statement; in brief:

1. ✅ **Seed staffing-self-consistency fix** (#113).
2. **SP5 autonomic-light agency, brought forward** *(updated 2026-06-28 — the 2026-06-26 "seed-coherence-first" Stage-1/Stage-2 detour below was **not** taken; #114 shipped the simpler housing-led seed and the logistics track resumed directly).* **Cut-1 shipped (#114):** housing-led seed + directed-logistics P1 (silent surplus→deficit redistribution) + autonomic-build (the recovery counterforce — self-funded, slow, no treasury). The track now continues with **P3 map overlay by `flowType` → P4 imports/exports dashboard** (model-agnostic UI on the silent flows), then the **economy-scaling + trade-logistics rework** (global `ECONOMY_SCALE` → calibrate via sim → contract-model rework → ship re-pricing; its doc is deleted with the grand-strategy pivot — the scale knob + calibration shipped, the contract/ship parts are retired). The at-risk middle needs food *moved* not rebuilt — logistics + build compose.
   <br>*Historical (not taken): an earlier 2026-06-26 plan re-sequenced SP5-light "seed-coherence-first" — Stage 1 (minimal subsistence-core seeder + build-planner-owned industry allocation + an age-forward snapshot harness growing the mature galaxy from live agency) → Stage 2 (emergent territory: un-owned space + dynamic membership + colonisation). The intent — "make the baseline honest, not efficient", grow the mature state from rules rather than hand-author it — survives as a future direction, but #114 took the housing-led seed and Stage 1/2 were shelved.*
