...
2. **SP5 autonomic-light agency, brought forward** *(updated 2026-06-28 — the 2026-06-26 "seed-coherence-first" Stage-1/Stage-2 detour below was **not** taken; #114 shipped the simpler housing-led seed and the logistics track resumed directly).* **Cut-1 shipped (#114):** housing-led seed + directed-logistics P1 (silent surplus→deficit redistribution) + autonomic-build (the recovery counterforce — self-funded, slow, no treasury). The track now continues with **P3 map overlay by `flowType` → P4 imports/exports dashboard** (model-agnostic UI on the silent flows), then the **economy-scaling + trade-logistics rework** (global `ECONOMY_SCALE` → calibrate via sim → contract-model rework → ship re-pricing; see [economy-scaling-and-trade-rework.md](./economy-scaling-and-trade-rework.md)). The at-risk middle needs food *moved* not rebuilt — logistics + build compose.
