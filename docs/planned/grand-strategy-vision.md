...
- **Packaging path**: A (pragmatic) — keep Next.js, world in local server memory, wrap with Electron for desktop. B (clean end state) — engine in a Web Worker, fully client-side, shippable as static web + Tauri/Electron desktop from one codebase. A migrates into B. The project runs on A today; B is booked on the roadmap under Platform, since everything else in this section has shipped and the packaging path is the only piece left open.
