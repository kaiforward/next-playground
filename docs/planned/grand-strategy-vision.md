...
1. **A living physical world, not a spreadsheet with a map.** Everything derives from universe generation and how factions use it (unchanged north star). Goods are produced by what a system physically is, consumed by the people who live there; systems genuinely thrive or starve. Price is a readout, not a reward. **No magic numbers, no designer-controlled outcomes where they can be avoided** — systems are built from physical primitives and distinct units (bodies with resource vectors, population, facilities that each produce one good, input-gated production chains) so realistic behaviour *emerges* from the simulation rather than being dialed in by hand. Calibration sets the *shape* of primitives (relative coefficients, per-capita needs) and lets the simulator find equilibrium; it never pins a specific result.
...
**Economy north star.** Nail three pillars first: population, industry, logistics. Genre DNA is Stellaris + EU5 (Project Caesar) + Victoria 3. The player is a faction *ruler*, not a trader — strip anything left over from the old browser space-trading game that doesn't serve that. Design linchpin: industry placement is driven by **demand (a rate/flow)**, not by filling a stock buffer.

**The ROI / `Proposal` rubric** is the durable review lens for any new faction decision (colonise-vs-build already arbitrate this way, on one shared per-faction throughput pool via ROI over a `build` | `colony_establish` union, gate-first — enablers raise cost, never value): a comparable value axis across kinds, COMPLETE cost including population and money (not just work), an open/additive `kind` discriminant, and per-doctrine coefficients rather than fixed formulae. Apply it to doctrine-driven allocation, priced warfare, and any future `Proposal` kind.
...
- **Staged system-level battles over a system's occupied prefix** — a hypothesis for the war brainstorm, not a commitment: a system's habitable bodies already sort best-first and population occupies them in that order (§ System Substrate & Habitability), so a system-level battle could stage itself the same way without any per-body population of its own — each occupied body one battle stage, worked outward from the weakest-held world toward the best, with the best world (the last-occupied, most defended) as the last stand. The fill order supplies both the stages and their sequence for free; nothing new needs simulating per body to get a multi-stage system fight out of it.
...
- **Packaging path B, decided and shipped**: the engine runs in a Web Worker, fully client-side, shippable as a static website — see [client-runtime.md](../active/engineering/client-runtime.md). The one piece still open is desktop packaging (Tauri vs Electron), booked on the roadmap under Platform; the worker-side engine extraction it depends on is already built.
- **Determinism**: seeded RNG, no wall-clock in the tick. Cheap to maintain now, and it keeps Paradox-style lockstep multiplayer possible *someday* without being designed for. Lockstep needs no DB and no stateful server — identical sims exchange only player commands via a stateless relay — so retiring Postgres closes no MP doors. Two guardrails were recorded at the time and still apply: a clean command boundary, and JS transcendental-`Math` producing different results across engines, which breaks lockstep between two clients on different runtimes. The doc holding the detail has since been deleted; git has it if the question reopens.
