...
> **Shipped 2026-06-18.** Faction-bounded topology + work-budget edge slicing landed on the shared branch (Phase A — structural refactor; Phase B — calibration + docs). Calibrated constants: `DISTANCE_DECAY = 0.1`, `EDGES_PER_TICK = 256` (a full sweep is 4 ticks at default scale, 46 at 10K — both inside the 200-tick prune window). A simulator sweep confirmed the targets hold (stocks in `[5,200]`, distance-graded dispersion on long-haul goods, greedy ≫ random) with no far-system starvation. The durable topology description now lives in [trade-simulation.md](../active/gameplay/trade-simulation.md); this section retains the design rationale below.
>
> **One correction to that rationale:** the distance-attenuation bullet imagined gateways as *low-friction* long-haul bridges. As built they are the opposite — gateways carry a 2.5× fuel multiplier (`GATEWAY_FUEL_MULTIPLIER`), so under `1/(1 + DISTANCE_DECAY·fuelCost)` they are **high-friction** and move *less*, not more. For goods diffusion this throttling is exactly what we want (it is what concentrates dispersion on long hauls). Whether Part 1 **migration** should instead bias *toward* gateways is a genuine open Part 1 question — it is **not** inherited from this topology, and migration would need its own gateway treatment to make them preferred routes.
