...
| Sustained Rationing never reaches collapse, at any tax (`:175-178`) | **re-authored: collapse never touches a Supplied or Strained world.** A world without famine at Provision ≥ `RATIONING_PROVISION` (banded Strained or better) cannot reach the 0.75 collapse line at any tax, any crowding, any override composition — worst case at the Strained band's lower edge is `0.23 + (0.95 + (0.30/0.75) × 1.45) × 0.30 = 0.689`. Collapse first becomes possible near shortfall 0.33 at max tax, just inside the Rationing band, and an untaxed world crosses it on its own near shortfall 0.67 — so the ladder tells the player the truth: Strained means struggling but nothing permanent can happen; Rationing or Shortage means the world can start dying. (The interim "shortfall ≤ 0.5 never collapses" wording proposed at the measurement gate is **false** under the override — a max-tax world at shortfall 0.5 concentrated in collapsed goods settles near 1.0 — and must not be resurrected. An earlier draft of this row also misnamed the protected zone "Rationing or better"; the 0.70–0.90 band is Strained.) |
