...
- **Substrate → Economy**: A system's bodies (resource deposits + build space) and population seed its industrial base at world-gen, which then drives capacity-driven, input-gated production each tick; consumption is population-scaled plus the production-input draw. Economy type is a derived display label. Narrative features carry no economic role
