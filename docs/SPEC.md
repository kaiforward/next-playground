...
Players manage a fleet of 12 ship classes across 3 sizes (small, medium, large) and 5 roles (trade, combat, scout, stealth, support). Each ship has 10 stats affecting travel, combat, and survival. Travel speed depends on ship speed stat (faster ships = fewer ticks). Ships can be grouped into convoys for collective travel and trade, with escort protection from all members (scaled by firepower — combat ships dominate). On arrival, cargo passes through a 5-stage danger pipeline: hazard incidents, import duty, contraband inspection, event-based cargo loss, and hull/shield damage. Ship stats and upgrade modules (12 modules across 4 slot types) modify danger outcomes through diminishing returns. Hull at 0 disables a ship (cargo lost, needs repair). Shields regenerate on dock.
