...
600 star systems across 24 regions (25 per region). Systems are connected by jump lanes forming a navigable graph. Each system has 2–4 traits (physical properties like asteroid belts, habitable worlds, precursor ruins) with quality tiers (1–3). Economy type (agricultural, extraction, refinery, industrial, tech, core) is derived bottom-up from trait affinities, not assigned directly. Each region has a government type and a dominant economy label. The map renders all systems simultaneously on a WebGL canvas with level-of-detail (LOD) — zoomed out shows system dots and Voronoi-derived region boundaries with large region name labels; zooming in progressively reveals system names, economy badges, ship counts, and event indicators. Gateway systems serve as chokepoints for inter-region travel.
...
Game clock advancing every 5 seconds. 7 processors run sequentially: ship arrivals, battles, events, economy, trade missions, operational missions, price snapshots. Processors are topologically sorted by dependencies. Economy processes one region per tick (round-robin). Results are broadcast to clients via SSE with per-player event filtering.
