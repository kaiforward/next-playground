...
3. **Accepted** — player claims the mission (POST `/api/game/op-missions/[missionId]/accept`). Mission reserved for player but no ship committed yet. Max 5 active operational missions per player.
4. **Started** — player assigns a ship and begins the mission (POST `/api/game/op-missions/[missionId]/start` with `{ shipId }`). Ship must pass stat gates, be docked at the mission's target system, not disabled, not in a convoy, and not committed to another mission.
   - Patrol/Survey/Salvage/Recon: mission set to `in_progress`, ship committed for duration (cannot navigate/trade)
   - Bounty: battle created at the `/start` call (not on ship arrival), ship locked until battle resolution
...
- **Actions each run**:
  ...
  2. Complete timed missions (patrol/survey/salvage/recon) where `startedAtTick + durationTicks ≤ currentTick` — credit player, free ship
...
Ship arrivals do not directly activate missions. Instead, mission activation uses the two-step accept/start API flow:
- **Accept**: Player claims the mission via `/accept` endpoint
- **Start**: Player assigns a ship via `/start` endpoint. For bounty missions, the battle is created at this point. For patrol/survey/salvage/recon, the mission starts its timed duration.
...
| `/api/game/op-missions/[missionId]/accept` | POST | Accept (claim) mission — reserves it for the player |
| `/api/game/op-missions/[missionId]/start` | POST | Start mission with `{ shipId }` — assigns ship, creates battle for bounty type |
