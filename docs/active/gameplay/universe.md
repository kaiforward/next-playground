...
- **Overlays** (multi-select) — Fleet, Events, Price, Trade-flow, Logistics, Ship Routes. Each row carries its glyph element's colour so the panel doubles as the key; the price-ramp, trade-flow tier, and routes legends appear in hover tooltips beside the row so they cost no permanent height.
