...
The Automation Module is currently a placeholder. The bonus bundle exposes `hasAutomation: boolean` but no processor consumes it. When automated trading ships, it will likely become a per-ship enabler with tiered behaviour gated on module tier. See `docs/planned/player-progression.md` for the design intent. `[PENDING: automation]`
