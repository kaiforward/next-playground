...
- **Claim (`unclaimed → controlled`)**: every faction scores the unclaimed systems exactly one jump (`REACH_JUMPS`) from any system it already owns — reach extends from controlled and developed systems alike, but never leapfrogs past a nearer unclaimed system to a richer one further out — and proposes its single best in-reach candidate. The score is an **absolute** read of the candidate's habitable space and resource diversity, discounted by proximity, so two factions' proposals for the same system compare directly rather than against each faction's own pool. The score floor is permissive to the point of being a no-op (0): a barren adjacent system with no habitable land or resource deposit is still claimable, it simply ranks last among that faction's candidates. When multiple factions propose the same system, the highest score wins the claim; an exact tie is broken by a single seeded-RNG draw over the tied factions in sorted order, so the outcome is fully determined by the world seed. The winning system becomes `controlled` under the winning faction — owned and border-closing, but still inert until developed.
