...
**Status: Implemented.** Engine in `lib/engine/expansion.ts`; wired into the directed-build processor's cycle pulse (`lib/tick/processors/directed-build.ts`, `lib/world/tick.ts`).

Each faction's territory grows outward from what it already owns, one step of each kind per cycle pulse, before that pulse's build phase runs:
