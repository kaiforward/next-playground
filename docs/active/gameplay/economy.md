...
- **Idle contraction (buffered, gentle).** Decay is a whole-level ratchet, not a continuous shave: a per-(system, building type) countdown accrues one reference-cycle each run while at least a whole level sits idle (`count > used`), and only once it crosses `idleBufferCycles` (120) does the marginal idle level tear down — the countdown resets the moment the level refills, so a brief dip costs nothing and only a sustained gap compounds down.
