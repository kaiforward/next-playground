...
3. **Output added** to local stock = `effectiveProduction_g × inputGate_g × ceiling`, where `ceiling` is the production brake — full rate at and below the warehouse knee (the larger of 40 cycles of the system's use figure and 8 cycles of its own capacity), ramping linearly to zero at 1.3 × the knee or the built physical storage, whichever is smaller.
