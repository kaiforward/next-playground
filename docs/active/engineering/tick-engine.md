...
| Infrastructure Decay | Cycle start | Economy | Shrinks `WorldBuilding.count` **downward only** toward what is *used* — disuse decay where built exceeds staffed-and-selling (`count × min(labourFulfillment, outputUptake)`) or housing occupancy (`population / POP_CENTRE_DENSITY`), plus a catastrophic unrest teardown above θ_decay. Runs only when the economy resolved this tick (reads its `economySignals` off `ctx.results`, incl. per-good output uptake), so it inherits the cycle start; applies `count` deltas in one pass and recomputes `popCap` live from the surviving housing. Never raises a count or goes below 0 |
...
| Directed Logistics | Cycle start | Economy | On the cycle boundary every faction redistributes surplus→deficit goods within its territory (`cycleStartShard`) — the sole mechanism that moves goods between systems. See [economy-autonomic-agency.md](../gameplay/economy-autonomic-agency.md) |
