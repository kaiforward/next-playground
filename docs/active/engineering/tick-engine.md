...
9 processors run sequentially each tick in topologically sorted order. Processors declare dependencies to ensure correct execution order. Two cadences interleave: a **per-tick heartbeat** (ship arrivals, events) advances every tick, while a **cycle resolution pulse** (economy → infrastructure decay → population → migration → directed logistics → directed build) resolves the whole galaxy together on the cycle boundary and no-ops on every other tick. See [Cadence](#cadence-per-tick-heartbeat--cycle-resolution-pulse).
...
| Economy | Cycle pulse | Events | On the cycle boundary (`tick % CYCLE_LENGTH === 0`) resolves the **whole galaxy** at once (`pulseShard`); no-ops otherwise. Applies event modifiers and government effects to each market's stock; applies strike suppression to production (derived from last cycle's `unrest`). Applied rates × `catchUpFactor` (= 1 at the reference interval → one full cycle's magnitude per resolution). Records per-system satisfaction (`delivered / demanded`) into `ctx.results` for infrastructure-decay and population |
| Infrastructure Decay | Cycle pulse | Economy | Shrinks `WorldBuilding.count` **downward only** toward what is *used* — disuse decay where built exceeds staffed-and-selling (`count × min(labourFulfillment, outputUptake)`) or housing occupancy (`population / POP_CENTRE_DENSITY`), plus a catastrophic unrest teardown above θ_decay. Runs only when the economy resolved this tick (reads its `economySignals` off `ctx.results`, incl. per-good output uptake), so it inherits the cycle pulse; applies `count` deltas in one pass and recomputes `popCap` live from the surviving housing. Never raises a count or goes below 0 |
| Population | Cycle pulse | Economy, Infrastructure Decay | Runs only when the economy resolved this tick (same `economySignals` gate → cycle). Reads per-system satisfaction from `ctx.results`; updates `unrest` (convex demand-weighted dissatisfaction integral); applies logistic population growth/decline against the **live** post-decay `popCap`; housing-overshoot (`population > popCap`, the unrest-snowball case) sheds the excess as unrest-weighted death (the conserved migration half rides the migration processor); rewrites `WorldMarket.demandRate` for each system's new population level |
| Migration | Cycle pulse | Population | On the cycle boundary relocates population (conserved) over the whole intra-faction open-edge topology (`pulseShard`); population flows down-unrest / up-headroom (`popCap − population`), distance-attenuated, per-edge amount × `catchUpFactor`. Gateways throttle migration as they do goods. Produces boom/bust geography over time |
| Directed Logistics | Cycle pulse | Economy | On the cycle boundary every faction redistributes surplus→deficit goods within its territory (`pulseShard`) — the sole mechanism that moves goods between systems. See [economy-autonomic-agency.md](../gameplay/economy-autonomic-agency.md) |
| Directed Build | Cycle pulse | Directed Logistics | On the cycle boundary every faction plans construction (`pulseShard`): proactive housing where population wants to grow, labour-gated industry where staffing and input self-supply support it. See [economy-autonomic-agency.md](../gameplay/economy-autonomic-agency.md) |
...
## Cadence: per-tick heartbeat + cycle resolution pulse
...
**Cycle resolution pulse — one tick a cycle.** On the cycle boundary (`tick % CYCLE_LENGTH === 0`, `CYCLE_LENGTH` = 24) the whole galaxy's faction-scale accounting resolves together, in dependency order: **economy → infrastructure decay → population → migration → directed logistics → directed build**. This is synchronized, not round-robin — every system's economy and every faction's logistics/build fire on the *same* tick, so a Paradox-style cycle settle replaces the old per-item rolling shard. Economy, migration, directed-logistics and directed-build gate on `pulseShard` (whole list on the boundary, empty otherwise); infrastructure-decay and population run only when the economy produced `economySignals` this tick, so they inherit the same cycle cadence automatically.
...
| **Cycle length** | ticks between whole-galaxy resolution pulses | `CYCLE_LENGTH` = 24 |
