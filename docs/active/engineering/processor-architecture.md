...
`World` (`lib/world/types.ts`) is schema-faithful flat rows and deliberately omits derived data some adapters expect inlined (a system's owning faction's `governmentType`, its building roster). `runWorldTick` performs those joins **once per tick** (`toTickSystems`/`toTickConnections`, exported so the harness's health analysers reuse them) before handing the views to the adapters. The joined rows are the tick's own working types (`lib/tick/rows.ts`) — mutable per-tick copies, merged back into the next `World` at the end of the tick.
...
Most processors resolve on a coarse **cycle** rather than every tick — the economy cluster each cycle, construction and logistics on their own intervals. The cycle interval is a real knob: `catchUpFactor(interval) = interval / REFERENCE_INTERVAL` (`lib/tick/shard.ts`) lets a body apply "elapsed-ticks worth" per run, so tuning an interval changes granularity, not wall-clock rate. `REFERENCE_INTERVAL` (24) is the calibration anchor and is **not** a knob — it is the divisor that makes the reference cadence `catchUp = 1` and behaviour-identical.

Every cycle-riding quantity is one of four shapes, and the shape dictates the treatment:
...
| **Counter / timer** — "after N cycles…" | decay's `idleCycles`, unrest-teardown collapse debt | accrue `catchUpFactor` per run (fractional); thresholds stay in reference-cycles |
| **Budget income** — "points per cycle" | construction pool / absorption cap / floor, logistics haul budget | multiply the income by `catchUpFactor` per run |
| **Target / stock / cost** — "fill to here", "costs this" | cycles-of-supply anchor, logistics delivery gap-fills, `workCostPerLevel`, build ceilings | **never scale** — levels are time-free |

The one-line rule: **scale flows and incomes by `catchUpFactor`; count time in ticks, not runs; never scale targets, stocks, or costs.** Logistics is the instructive hybrid — its per-cycle haul *budget* scales, but its deliveries are gap-fills toward the anchor and must not (filling more often already means smaller fills; scaling them too would overshoot the anchor).
...
**Two cadence clusters, not one clock.** Population, infrastructure-decay, and migration read the economy's same-tick signal and process its shard, so they are welded to the economy's `CYCLE_LENGTH` — one societal accounting pass. Directed-build and directed-logistics are self-gated on their own `cycleStartShard(interval)` and coupled only through world state, so they carry independent knobs (`CONSTRUCTION_INTERVAL`, `LOGISTICS_INTERVAL`). Relations runs on its own `RELATIONS_FREQUENCY` (3) with the same latent property: its per-run drift magnitudes are denominated in that frequency and would need the same treatment if it were ever tuned.

**Testing.** Interval invariance is directly checkable — same seed and span, different interval, matching wall-clock rates — but only statistically: cycles land on different ticks (different RNG draws) and `fundQueue`'s non-homogeneous `remaining` term redistributes construction, so exact parity is impossible. `runWorldTick(world, { cadence })` takes an optional per-run override (the live loop never sets it; the harness threads it from an experiment's `cadence:` block) so the gate can run both cadences: `lib/world/__tests__/cadence-invariance.test.ts` for the societal + construction rates, and the full-scale `experiments/examples/cadence-invariance-*.yaml` pair for the budget-bound logistics regime CI can't reach.
